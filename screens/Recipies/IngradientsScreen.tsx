...
export default function IngradientScreen({ navigation }) {
    ...
    useEffect(() => {
        feed.ingredients && setValue(feed.ingredients.map(el => `${el.amount} ${el.unit} ${el.name}`).join('\n'))
    }, [feed])
        ...
        <ScrollView contentContainerStyle={{ flex: 1 }}>
            <View style={{ flex: 1 }}>
                {!editMode ?
                    <View  >
                        <View>
                            {feed.ingredients && feed.ingredients.map((el, index) => {
                                return (
                                    <IngradientRow
                                        key={index}
                        ...
                        <View style={{ ...styles.btnConatiner, marginTop: 8 }}>
                            <Button
                                ...
                                fts={Typ.Small}
                                ftw={'500'}
                                disabled={!Object.values(checked).filter(el => el).length ? true : false}
                            />
                        </View>

...
export default function PersonalDataScreen({ navigation }) {
    ...
    const getProfileData = async () => {
        const { data: { intakeNorms: norms, preferences } } = await server.getProfile();
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
                <View style={styles.container}>
                        <View style={styles.intakeNormContainer}>
                            <Text style={styles.intakeNormText}>
                                Daily intake norm
                            </Text>
                            {console.log(isEnabled, 'dsgdfgfdgfdgdfjgi34')}
                            <View>
                                {Object.keys(intakeNorm).sort((a, b) => intakeNorm[a].editable < intakeNorm[b].editable ? 1 : -1 ).map((el: string) => (
                                    <View style={styles.normItemContainer}>
                                        <Text>
                                            {intakeNorm[el].title}
                                        </Text>
                                        {!intakeNorm[el].editable ? 
                                            <View style={{paddingVertical: 11,  width: '50%', paddingHorizontal: 19}}>
                                                <Text >
                                                {intakeNorm[el].value}
                                                </Text>
                                            </View> 
                                        :
                                            ...
                                            defaultIcon={intakeNorm[el].editable ? 'keyboard-arrow-up' : undefined}
                                            onBlur={blurHandler}
                                            defaultIconSize={21}
                                            additionalIcon={intakeNorm[el].editable ? 'keyboard-arrow-down' : undefined}

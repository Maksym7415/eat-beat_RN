...
const RecommendedScreen: FC<NavProps> = ({ navigation, clear }) => {
  ...
  const serveData = async () => {
    ...
    const response = await server.getRecommendedRestaurant(date);
    if (response.ok) {
  ...
  const addMeal: AddMealsFun = async (id, { creationTime, servings }) => {
    ...
    await server.addRestaurantsMeal(data);
    setFetching({ clicked: false, deactivate: false });
  ...
  const onPreview = async (item) => {
    ...
    navigation.navigate('previewRestaurantScreen', {
      title: name,
      details: {...details, page: 'restaurants', fromPage: 'recommendedRestaurant'},
  ...
  return feed?.length ? (
    ...
    <View style={styles.noRecommendationContainer}>
        ...
        <Button
            ...
            isShow={true}
            clicked={fetching.clicked}
            style={{ backgroundColor: Col.Restaurants }}
        />
        </View>
    </>
  );
